...
enum open_flags {
    ...
    */
    UFS_READ_WRITE = 6,
    /**
        With this flag it allowed to writing at the end of
        file.
    */
    USF_APPEND = 8
#endif
...
/**
    \brief  Open a file by filename.
    \param  [in]  filename  Name of a file to open.
    \param  [in]  flags     Bitwise combination of open_flags.
    \retval > 0 File descriptor.
            -1 Error occurred. Check ufs_errno() for a code.
...
/**
    ...
    \param  [in]  fd    File descriptor from ufs_open().
...
/**
    \brief  Delete a file by its name.
    \param  [in]  filename  Name of a file to delete.
    \retval -1 Error occurred. Check ufs_errno() for a code.
            - UFS_ERR_NO_FILE - no such file.
    \note   Note, that it is allowed to drop the
...
/**
    \brief   Resize a file opened by the file descriptor a fd.
    \details If current file size is less than a new_size, then
             new empty blocks are created and positions of opened file
             descriptors are not changed. If the current size is bigger
             than a new_size, then the blocks are truncated. Opened file
             descriptors behind the new file size should proceed from the
             new file end.
    \param   [in]  fd  File descriptor from ufs_open().
    \param   [in]  new_size  New file size.
    \retval  0 Success.
